...
metrics = {"heights_done": 0, "errors": 0, "touched_addresses": 0}
...
async def get_block_meta(session, height: int) -> Tuple[str, datetime.datetime]:
    ...
    block = await with_retry(rpc_call, session, "getblock", [block_hash, 1], timeout=60.0)
...
async def get_next_address_jobs(batch_size: int) -> List[int]:
    async with pool.connection() as conn:
        async with conn.transaction():
            async with conn.cursor() as cur:
                # Aligns with your partial index block_jobs_addr_sched_idx
                await cur.execute("""
                    UPDATE blockchain.block_jobs
...
async def get_addr_progress() -> Tuple[int, int, int, int, int]:
    async with pool.connection() as conn:
        async with conn.cursor() as cur:
            await cur.execute("""
                SELECT
                  COUNT(*) FILTER (WHERE address_status = 'done'::blockchain.block_job_status)        AS done,
                  COUNT(*) FILTER (WHERE address_status = 'in_progress'::blockchain.block_job_status) AS in_progress,
                  COUNT(*) FILTER (WHERE address_status = 'pending'::blockchain.block_job_status)     AS pending,
                  COUNT(*) FILTER (WHERE address_status = 'failed'::blockchain.block_job_status)      AS failed,
                  COALESCE(MAX(height) FILTER (WHERE address_status = 'done'::blockchain.block_job_status), 0)
                    AS latest_done_height
                FROM blockchain.block_jobs
    ...
    try:
        ...
        async with db_write_sem:
            async with pool.connection() as conn:
                async def _write():
                    async with conn.transaction():
                        # Build per-address deltas entirely in SQL.
                        # We only consider addresses already present in watch_addresses.
                        async with conn.cursor() as cur:
                            await cur.execute("""
                                WITH block_txs AS (
                                  SELECT txid
                                ...
                                -- Lock watch rows deterministically to prevent deadlocks
                                locked AS (
                                  SELECT wa.address
                            ...
                            touched_count = len(updated_rows)

                            # No watched addresses touched? Still mark the height as done.
        ...
        metrics["heights_done"] += 1
        metrics["touched_addresses"] += touched
...
async def monitor_loop(session: aiohttp.ClientSession):
    ...
    while not shutdown_event.is_set():
        try:
            done, in_progress, pending, failed, latest_done_height = await get_addr_progress()
            ...
            if start_done is not None and done > start_done:
                ...
                eta = datetime.datetime.now(datetime.UTC) + datetime.timedelta(seconds=(pending / rate)) if rate > 0 else datetime.datetime.now(datetime.UTC)
            ...
            log.info("progress", line=2,
                     ...
                     touched_addresses=metrics["touched_addresses"],
                     heights_done=metrics["heights_done"],
                     errors=metrics["errors"])

            if pending == 0 and in_progress == 0:
                log.info("addr_all_blocks_processed", metrics=metrics)
                shutdown_event.set()
        except Exception as e:
